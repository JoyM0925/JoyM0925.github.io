...
let reach = 150;// Gow far connectioms b/w
...
class csNode{
  //1. constructor
  constructor(x, y){// called once per objected created
  // properties related to positions/rendering
    this.x = x;
    this.y = y;
    this.size = 20;
  ...
  connect(nodeArray){
    //check if the curremt point is close to any other points and if so join with a line
    stroke(this.c);
    for(let n of nodes){
  ...
  move(){
    ...
    let ySpeed = noise(this.yTime); //0-0.5-1
